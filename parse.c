...
static Node *new_vla_ptr(Var *var, Token *tok) {
...
static Node *to_assign(Node *binary) {
  ...
  if (binary->lhs->kind == ND_MEMBER) {
    Var *var = new_lvar("", pointer_to(binary->lhs->lhs->ty));
  ...
  if (binary->lhs->ty->is_atomic) {
    ...
    Var *addr = new_lvar("", pointer_to(binary->lhs->ty));
    Var *val = new_lvar("", binary->rhs->ty);
    Var *old = new_lvar("", binary->lhs->ty);
    Var *new = new_lvar("", binary->lhs->ty);
...
static Var *find_func(char *name) {
  Scope *sc = scope;
  while (sc->next)
    sc = sc->next;
...
static void mark_live(Var *var) {
  if (!var->is_function || var->is_live)
    return;
  var->is_live = true;

  for (int i = 0; i < var->refs.len; i++) {
    Var *fn = find_func(var->refs.data[i]);
    if (fn)
      mark_live(fn);
  }
}
