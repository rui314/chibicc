...
// to solve issue #117 I had to add 0--9 range to be allowed because some generics parameters
// are defined after ## (double hashtag). Exemple in openssl ...(void))fromtype##2obj_decode},
// In this case chibicc manages 2 as a number because identifier should not start by a numeric value!
// creating is_ident3 to manage it
...
// For now the solution I found is to add here the range 0-9 has valid character for identifier.
// it means that some incorrect values would be allowed. To avoid it, I need to check if the previous character
// is hashtag to allow in this case an identifier starting with numeric character
bool is_ident3(uint32_t c)
{
  static uint32_t range[] = {
      '_',
      '_',
